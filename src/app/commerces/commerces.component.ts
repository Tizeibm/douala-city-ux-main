...
export class CommercesComponent implements OnInit {
  ...
  categories: { nom: string; sousCategories: string[] }[] = [
    { nom: 'Commerce', sousCategories: ["Alimentation", "Vêtements", "Électronique", "Meubles", "Livres", "autres"] }
  ];
  ...
  getSousCategories(): string[] {
    const cat = this.categories.find(c => c.nom === 'Commerce');
  ...
  loadStructures() {
    this.loading = true;
    this.entrepriseService.getByCategorie('Commerce', this.currentPage - 1, this.itemsPerPage).subscribe({
      next: data => {
