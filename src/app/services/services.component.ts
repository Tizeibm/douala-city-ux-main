...
export class ServicesComponent implements OnInit {
  ...
  getSousCategories(): string[] {
    const cat = this.categories.find(c => c.nom === 'services');
    this.sc = cat ? cat.sousCategories : [];
    return this.sc;
  }
  ...
  loadStructures() {
    this.loading = true;
    this.entrepriseService.getByCategorie('services', this.currentPage - 1, this.itemsPerPage).subscribe({
      next: data => {
