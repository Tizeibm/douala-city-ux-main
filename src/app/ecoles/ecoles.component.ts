...
@Component({
  ...
  styleUrl: './ecoles.component.scss'
})
export class EcolesComponent implements OnInit {
  ...
  getSousCategories(): string[] {
    const cat = this.categories.find(c => c.nom === 'Education');
  ...
  loadStructures() {
    this.loading = true;
    this.entrepriseService.getByCategorie('Education', this.currentPage - 1, this.itemsPerPage).subscribe({
      next: data => {
